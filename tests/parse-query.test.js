...
describe("ParseQueryFilter runs as expected when", () => {
  test("given a simple eq filter with one statement", () => {
    ...
    assert.deepStrictEqual(result, { $and: [{ name: { $eq: "John Doe" } }] })
  })
  ...
  test("given a simple or filter with two statements, where one value has number", () => {
    ...
    assert.deepStrictEqual(result, { $or: [{ name: { $eq: "John Doe" } }, { age: { $ne: 42 } }] })
  })
  ...
  test("given a not filter with a nested or inside, and string eq without single quotes", () => {
    ...
    assert.deepStrictEqual(result, {
      $nor: [
        {
          $or: [{ name: { $eq: "Petter" } }, { hobby: { $ne: "pizza" } }]
        },
        {
          $and: [{ age: { $eq: 42 } }]
        }
      ...
      $nor: [
        {
          $and: [
            ...
            {
              $or: [{ duplicate: { $eq: true } }, { stilig: { $eq: null } }]
            }
          ]
        },
        {
      ...
      $nor: [
        {
          $and: [
            { createdTimestamp: { $gt: "2024-02-24" } },
            {
              $or: [{ duplicate: { $eq: "false" } }, { stilig: { $eq: "null" } }]
            }
          ]
        },
        {
      ...
      $nor: [
        {
          $and: [
            { createdTimestamp: { $gt: "2024-02-24" } },
            {
              $or: [{ stuff: { $eq: "tipp or tapp" } }, { stilig: { $eq: null } }]
            }
          ]
        },
        {
          $or: [{ dish: { $eq: "and i saus" } }, { hobby: { $ne: "pizza" } }]
        }
      ]
    })
  })
      ...
      $nor: [
        {
          $and: [
            { createdTimestamp: { $gt: "2024-02-24" } },
            {
              $or: [{ "tipp or tapp": { $eq: true } }, { stilig: { $eq: null } }]
            }
          ]
        },
        {
          $or: [{ "and i saus": { $lt: 4 } }, { hobby: { $ne: "pizza" } }]
        }
      ]
    })
  })
...
describe("ParseQueryFilter throws error when", () => {
  test("using several logical operators on the same level", () => {
    ...
    assert.throws(shouldThrow, /You must use parenthesis when using multiple logical operators in a filter/)
  })
  ...
  test("a statement is malformed", () => {
    ...
    assert.throws(shouldThrow, /Statement must be on the form <property> <operator> <value>/)
  })
  ...
  test("using unknown logical operator", () => {
    ...
    assert.throws(shouldThrow, /You must use exactly one operator in a statement/)
  })
  ...
  test("property name has whitespace and not using single quotes", () => {
    ...
    assert.throws(shouldThrow, /Property names with whitespace must be enclosed in single quotes/)
  })
  ...
  test("value has whitespace and not using single quotes", () => {
    ...
    assert.throws(shouldThrow, /Values with whitespace must be enclosed in single quotes/)
  })
  ...
  test("using empty filter", () => {
    ...
    assert.throws(shouldThrow, /Filter must contain at least one statement or one sub filter/)
  })
