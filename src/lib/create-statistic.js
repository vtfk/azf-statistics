...
const createStatistic = (data) => {
  // Går gjennom det som er sendt inn - sjekker at alle required er med, og riktig type. Feilmelding hvis ikke
  const missingFields = []
  const errorTypes = []

  for (const [prop, value] of Object.entries(requiredFields)) {
    const propName = prop.replace("?", "")
    if (data[propName] && typeof data[propName] !== typeof value) {
      errorTypes.push(`${propName} must be "${typeof value}"`)
    }
    if (!prop.startsWith("?") && !data[propName]) {
      missingFields.push(`"${propName}"`)
    }
  }

  if (missingFields.length > 0 || errorTypes.length > 0) {
    throw new Error(`Missing required fields or type in body. Missing fields: [${missingFields.join(", ")}]. Error types: [${errorTypes.join(", ")}]`)
  }
  ...
  for (const [prop, value] of Object.entries(data)) {
    if ((value !== null && typeof value === "object") || typeof value === "undefined") {
      notFlat.push(`Illegal type "${typeof value}" found in property "${prop}"`)
    }
  }

  if (notFlat.length > 0) {
    throw new Error(notFlat.join(", "))
  }
