...
const parseFilterPart = (part) => {
  ...
  for (let i = 0; i < part.length; i++) {
    ...
    if (part.charAt(i) === " ") {
      // If we are at a space, we need to check if we have an operator
      for (const operator of operators) {
        if (part.substring(i + 1, i + operator.length + 2) === `${operator} ` && !isInsideQuotes) {
          // We don't want to find logical operators inside quotes, e.g. story: 'I took the book and the bird'
  ...
  if (foundOperators.length === 0) {
    throw new Error(`Statement must be on the form <property> <operator> <value>. Statement: "${part}"`)
  }

  if (foundOperators.length !== 1) {
    throw new Error(`You must use exactly one operator in a statement. Statement: "${part}"`)
  }

  if (property.length === 0) {
    throw new Error(`You must provide a property in a statement. Statement: "${part}"`)
  }

  if (value.length === 0) {
    throw new Error(`You must provide a value in a statement. Statement: "${part}"`)
  }
  ...
  if (!quoteTrimmedProperty.isInsideQuotes) {
    ...
    if (property.includes(" ")) {
      throw new Error(`Property names with whitespace must be enclosed in single quotes. Property: "${property}"`)
    }
  }
  ...
  if (!quoteTrimmedValue.isInsideQuotes) {
    if (value.includes(" ")) {
      throw new Error(`Values with whitespace must be enclosed in single quotes. Value: "${value}"`)
    }

    // Check if value is a number, and if so, parse it to a number
    // biome-ignore lint/suspicious/noGlobalIsNan: Allow use of isNaN for legacy compatibility
    if (!isNaN(value)) {
      value = Number(value)
    }
...
const parseQueryFilter = (filter) => {
  ...
  for (let i = 0; i < filter.length; i++) {
    if (filter.charAt(i) === "(") {
      // If we are inside parenthesis, we need to build a subpart
      level++

      if (level === 1) {
        continue // We don't want to add the first parenthesis (level 1) to the subpart, but others we need to add, in case there are nested parenthesis
      }
    }

    if (filter.charAt(i) === ")") {
      if (level === 1) {
        // If we are at the end of a subpart, we need to push the current subpart to the sub parts array
      ...
      level--
      if (level === 0) {
        continue // We don't want to add the last parenthesis (level 0) to the subpart, but nested parenthesis we need to add
      }
    }
    ...
    if (level === 0) {
      ...
      if (filter.charAt(" ")) {
        // If we are at a space, we need to check if we have a logical operator
        for (const logicalOperator of logicalOperators) {
          if (filter.substring(i + 1, i + logicalOperator.length + 2) === `${logicalOperator} ` && !isInsideQuotes) {
            // We don't want to find logical operators inside quotes, e.g. story: 'I took the book and the bird'
            foundLogicalOperators.push(logicalOperator)

            if (currentPart.trim().length > 0) {
              parts.push(currentPart) // Don't need to push empty parts
            }
    ...
    if (level > 0) {
      currentSubPart += filter.charAt(i) // If we are inside parenthesis, we need to add the char to the current subpart
    }

    if (i === filter.length - 1 && currentPart.trim().length > 0) {
      parts.push(currentPart) // If we are at the end of the filter, we need to push the current part to the parts array
    }
  }

  if (!foundLogicalOperators.every((operator) => operator === foundLogicalOperators[0])) {
    throw new Error(`You must use parenthesis when using multiple logical operators in a filter. Filter: "${filter}"`)
  }

  const logicalOperator = foundLogicalOperators[0] ? `$${foundLogicalOperators[0]}` : "$and"

  if (parts.length === 0 && subParts.length === 0) {
    throw new Error(`Filter must contain at least one statement or one sub filter. Filter: "${filter}"`)
  }
