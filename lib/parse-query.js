...
const parseFilterPart = (part) => {
  ...
  if (!quoteTrimmedProperty.isInsideQuotes) {
    // Check if contains whitepace - throw error if so
...
const parseQueryFilter = (filter) => {
  ...
  if (parts.length === 0 && subparts.length === 0) throw new Error(`Filter must contain at least one statement or one subfilter. Filter: "${filter}"`)
